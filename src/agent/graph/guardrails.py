...
def system_prompt():
    return (
        ...
        "editor applies it.\n"
        "- Whenever you name an event in your reply, wrap its title in a mention "
        "marker of the form [[id|Title]], for example "
        "\"1. [[6713287|Title]] — date, location\". Use the marker every time you "
        "refer to that event, and use it instead of writing the id separately — "
        "never show the same id both inside a marker and as \"(id ...)\". Put only "
        "the plain title in the marker; no markdown, and no | or ] characters "
        "inside it.\n"
