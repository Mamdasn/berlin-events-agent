...
def test_bold_quoted_duplicate_marker_collapses():
    surfaced = {1: {"event_id": 1, "title": "Odd ritual"}}
    text = build._finalize_event_mentions(
        '{{event:1}} — **"Odd ritual"** is the strongest pick.', surfaced
    )
    assert text == "[[1|Odd ritual]] is the strongest pick."


def test_german_quoted_duplicate_marker_collapses():
    surfaced = {1: {"event_id": 1, "title": "Odd ritual"}}
    text = build._finalize_event_mentions(
        "{{event:1}} — „Odd ritual“ ist der stärkste Kandidat.", surfaced
    )
    assert text == "[[1|Odd ritual]] ist der stärkste Kandidat."


def test_quote_style_paraphrase_of_title_still_collapses():
    surfaced = {
        2: {"event_id": 2, "title": 'Kindertheater "Malwina und der Erdbeerpilz"'}
    }
    text = build._finalize_event_mentions(
        "{{event:2}} — \"Kindertheater 'Malwina und der Erdbeerpilz'\" (16:00).",
        surfaced,
    )
    assert text == '[[2|Kindertheater "Malwina und der Erdbeerpilz"]] (16:00).'


def test_quoted_echo_after_meta_parens_is_dropped():
    surfaced = {1: {"event_id": 1, "title": "Odd ritual"}}
    text = build._finalize_event_mentions(
        "{{event:1}} (10:00–20:00, Galerie im Körnerpark, Neukölln) — "
        '"Odd ritual" is a contemporary art exhibition.',
        surfaced,
    )
    assert text == (
        "[[1|Odd ritual]] (10:00–20:00, Galerie im Körnerpark, Neukölln) "
        "is a contemporary art exhibition."
    )


def test_quoted_echo_keeps_following_dash_clause():
    surfaced = {1: {"event_id": 1, "title": "Odd ritual"}}
    text = build._finalize_event_mentions(
        '{{event:1}} (10:00, Villa) — "Odd ritual" — a solo exhibition.',
        surfaced,
    )
    assert text == "[[1|Odd ritual]] (10:00, Villa) — a solo exhibition."
...
def test_prompt_explains_handle_renders_as_title():
    prompt = guardrails.system_prompt()
    assert "replaces each handle with the event's full title" in prompt
    assert "never write the title next to a handle" in prompt
